...
bool producting(int argc, char* argv[])
{
	bool answer = false;
	...
	else //there is interesting information
	{
		...
		double maxTreas;
		int i=1;
		for( i = 1; i<argc; i++) //don't want to read argv[0]
		{//argv[i] is a string
			...
			case 1:
				//this is filename
				printf("The length of the filename is %ld.\n",strlen(argv[i]));
				printf("The proposed filename is %s.\n", argv[i]);
				if(strlen(argv[i])>=argc-1)
				{
...
bool readFile(char* filename, int* nrooms, AdjMat* adjMP, Room** theRoomPs)
{
	...
	else
	{
		...
		int roomr=1;
		int roomc = 0;
		for( roomr = 1; roomr<*nrooms; roomr++)
		{
			printf("on row %d\n", roomr);fflush(stdout);
			roomc = 0;
			for(roomc = 0; roomc<roomr; roomc++)
			{
				fscanf(fp,"%d", &temp);
		...
		float tempTreas = 2.9;
		roomr=0;
		for(roomr = 0; roomr<*nrooms; roomr++)
		{
